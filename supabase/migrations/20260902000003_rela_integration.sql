-- ============================================================
-- MIGRATION: Open RELA Argentina â€” IntegraciÃ³n de portal inmobiliario
...
-- ------------------------------------------------------------
-- 1. ESTADOS DE PUBLICACIÃ“N (estados locales de la integraciÃ³n.
--    El estado REMOTO real de RELA es ONLINE/OFFLINE/EN_ESPERA y
...
DO $$ BEGIN
  CREATE TYPE rela_listing_status AS ENUM (
    ...
    'SYNCING',          -- hay una operaciÃ³n en curso
    'PUBLISHED',        -- RELA confirmÃ³ ONLINE
    'UPDATE_PENDING',   -- la propiedad cambiÃ³ en BH (hash difiere) y hay que re-enviar
    'UPDATING',         -- actualizaciÃ³n en curso en RELA
    'UNPUBLISHED',      -- RELA confirmÃ³ OFFLINE (baja)
    'ERROR',            -- Ãºltima operaciÃ³n fallÃ³ (ver last_error)
    'BLOCKED'           -- validaciÃ³n local no superada; no se envÃ­a a RELA
  );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- ------------------------------------------------------------
-- 2. CONFIGURACIÃ“N (datos NO secretos; client_id/secret van en
...
CREATE TABLE IF NOT EXISTS public.rela_config (
  ...
  codigo_inmobiliaria TEXT,              -- lo entrega QuintoAndar tras asociar la inmobiliaria
  integrador TEXT,                       -- parÃ¡metro INTEGRADOR del botÃ³n de asociaciÃ³n
  plan_default TEXT NOT NULL DEFAULT 'SIMPLE',
  contacto_nombre TEXT,
  contacto_email TEXT,
  contacto_telefono TEXT,
  callbacks_enabled BOOLEAN NOT NULL DEFAULT FALSE,
  -- Mapeos internos â†’ catÃ¡logos RELA. VacÃ­os hasta que el admin los configure
  -- con los IDs reales devueltos por GET /v1/tipopropiedades y
  -- GET /v1/tipopropiedades/{id}/caracteristicas (NO hardcodear IDs).
  catalog_mapping JSONB NOT NULL DEFAULT '{}'::jsonb,   -- {SUPERFICIE_TOTAL:"CFT100", SUPERFICIE_CUBIERTA:"CFT101", AMBIENTES:"â€¦", DORMITORIOS:"â€¦", BANOS:"â€¦", MEDIO_BANO:"â€¦", GARAGE:"â€¦"}
  tipo_propiedad_map JSONB NOT NULL DEFAULT '{}'::jsonb, -- {departamento:{idTipo,idSubTipo}, casa:{â€¦}, â€¦}
  ubicacion_map JSONB NOT NULL DEFAULT '{}'::jsonb,      -- {zona_normalizada: idUbicacion RELA}
  dry_run BOOLEAN NOT NULL DEFAULT TRUE, -- TRUE hasta que el admin lo apague explÃ­citamente
...
CREATE TABLE IF NOT EXISTS public.rela_listings (
  ...
  codigo_aviso TEXT NOT NULL UNIQUE,           -- identificador Ãºnico que BH asigna al aviso
  id_aviso_navplat BIGINT,                     -- idAviso devuelto por RELA
  status rela_listing_status NOT NULL DEFAULT 'PENDING',
  remote_status TEXT,                          -- ONLINE / OFFLINE / EN_ESPERA (visceral de RELA)
  plan TEXT,                                   -- tipoDePublicacion usado
  payload_hash TEXT,                           -- sha256 del payload normalizado (detecciÃ³n de cambios)
  remote_updated_at TIMESTAMPTZ,               -- fechaModificacion del resumen online
  published_at TIMESTAMPTZ,
  unpublished_at TIMESTAMPTZ,
  last_sync_at TIMESTAMPTZ,
  last_error TEXT,
  last_warnings JSONB,                         -- warnings no fatales de la Ãºltima operaciÃ³n
  remote_snapshot JSONB,                       -- Ãºltima respuesta de GET aviso (debug)
...
-- ------------------------------------------------------------
-- 5. CACHÃ‰ DE CATÃLOGOS (ubicaciones, tipos, planes, operacionesâ€¦)
-- ------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS public.rela_webhook_events (
  ...
  referencia TEXT,                       -- codigoAviso asociado (si RELA lo envÃ­a)
  id_aviso_navplat BIGINT,
  payload JSONB NOT NULL,
  processed BOOLEAN NOT NULL DEFAULT FALSE,
  process_error TEXT,
  lead_id UUID,                          -- si generÃ³ un lead en CRM
...
-- config: lectura autenticados; escritura sÃ³lo super_admin
...
-- listings: lectura autenticados; escrituras SOLO vÃ­a service_role (edge functions)
...
-- rela_tokens y rela_webhook_events: SIN polÃ­ticas â†’ Ãºnicamente service_role.
...
AS $$
DECLARE
  v_config rela_config%ROWTYPE;
  v_counts RECORD;
  v_result JSONB;
BEGIN
  SELECT * INTO v_config FROM rela_config WHERE id = TRUE;
