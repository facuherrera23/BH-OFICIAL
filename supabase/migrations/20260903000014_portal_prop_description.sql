-- ============================================================
-- 20260903000014_portal_prop_description
-- Agrega `description` al payload de cada propiedad en
-- portal_get_portal_data para enriquecer el detalle expandido
-- del apartado "Mis Propiedades" del portal.
-- Redefine la RPC (CREATE OR REPLACE). No toca schema de tablas.
-- ============================================================
...
DECLARE
  ...
  v_excl_start   date;
  v_excl_end     date;
  v_excl_leads   integer := 0;
  v_excl_visits  integer := 0;
  v_excl_vdone   integer := 0;
  v_excl_days    integer;
  v_prev_start   date;
  v_prev_end     date;
  v_prev_leads   integer := 0;
  v_prev_visits  integer := 0;
  v_prev_vdone   integer := 0;
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(t.js), '[]'::jsonb) INTO v_properties
    FROM (
      SELECT jsonb_build_object(
               'id', p.id, 'property_code', p.property_code, 'title', p.title, 'description', p.description,
               'zone', p.zone, 'address', p.address,
  ...
  v_excl_start := (v_owner->>'exclusive_start')::date;
  v_excl_end := (v_owner->>'exclusive_end')::date;
  IF v_excl_start IS NOT NULL THEN
    SELECT COUNT(*) INTO v_excl_leads
      FROM public.leads l
      JOIN public.properties p ON p.id = l.property_id
     WHERE p.owner_id = v_owner_id
       AND p.deleted_at IS NULL
       AND l.created_at::date >= v_excl_start
       AND l.created_at::date <= COALESCE(v_excl_end, now()::date);

    SELECT COUNT(*), COUNT(*) FILTER (WHERE v.status = 'completada')
      INTO v_excl_visits, v_excl_vdone
      FROM public.visits v
      JOIN public.properties p ON p.id = v.property_id
     WHERE p.owner_id = v_owner_id
       AND p.deleted_at IS NULL
       AND v.visit_date::date >= v_excl_start
       AND v.visit_date::date <= COALESCE(v_excl_end, now()::date);
  END IF;

  -- Comparativa del período anterior de igual duración
  IF v_excl_start IS NOT NULL AND v_excl_end IS NOT NULL THEN
    v_excl_days := GREATEST(v_excl_end - v_excl_start, 1);
    v_prev_end := v_excl_start - 1;
    v_prev_start := v_prev_end - v_excl_days + 1;
    ...
    SELECT COUNT(*) INTO v_prev_leads
     ...
     WHERE p.owner_id = v_owner_id
       AND p.deleted_at IS NULL
       AND l.created_at::date >= v_prev_start
       AND l.created_at::date <= v_prev_end;

    SELECT COUNT(*), COUNT(*) FILTER (WHERE v.status = 'completada')
      INTO v_prev_visits, v_prev_vdone
      FROM public.visits v
      JOIN public.properties p ON p.id = v.property_id
     WHERE p.owner_id = v_owner_id
       AND p.deleted_at IS NULL
       AND v.visit_date::date >= v_prev_start
       AND v.visit_date::date <= v_prev_end;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'excl_stats', jsonb_build_object(
      'leads', v_excl_leads, 'visits', v_excl_visits, 'visits_done', v_excl_vdone,
      'period_start', v_excl_start, 'period_end', v_excl_end, 'period_days', v_excl_days,
      'prev_leads', v_prev_leads, 'prev_visits', v_prev_visits, 'prev_visits_done', v_prev_vdone,
      'prev_period_start', v_prev_start, 'prev_period_end', v_prev_end
    )
  );
END;
$function$;
