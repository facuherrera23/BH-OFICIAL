-- Portal Propietario: detalle de propiedades
-- Agrega address, surfaces, rooms/bedrooms/bathrooms/garage_spaces, video_url,
-- image_urls y stats por propiedad (leads/visitas) a portal_get_portal_data.
  ...
  SELECT COALESCE(jsonb_agg(t.js), '[]'::jsonb) INTO v_properties
    FROM (
      SELECT jsonb_build_object(
               ...
               'is_published', p.is_published, 'featured', p.featured,
