-- Portal Propietario: RPC portal_get_portal_data (versión inicial con stats)
-- Snapshot único del portal: owner (sin datos bancarios), propiedades,
-- leads por fuente, totales de leads y visitas, y usd_rate.
-- SECURITY DEFINER con search_path fijo; ejecutable por anon/authenticated.
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(t.js), '[]'::jsonb) INTO v_properties
    FROM (
      SELECT jsonb_build_object(
               'id', p.id, 'property_code', p.property_code, 'title', p.title, 'zone', p.zone,
               'property_type', p.property_type::text, 'status', p.status::text, 'price_usd', p.price_usd,
               'price_currency', p.price_currency, 'is_published', p.is_published, 'featured', p.featured,
